...
# Link corretto e certificato con forcing anti-congelamento
...
if df is not None:
    ...
    with tab_oggi:
        # Lettore dinamico sulla colonna B (indice 1) del tuo foglio reale
        def prendi_dato(riga_foglio, valore_di_prova):
            try:
                valore = str(df.iloc[int(riga_foglio) - 1, 1]).strip()
                if valore == "nan" or valore == "" or len(valore) > 20: 
        ...
        st.markdown(f"""
            <div class="metric-card bg-verde">
                ...
                <div class="metric-status">🟢 Calcolato in tempo reale</div>
            </div>
        """, unsafe_allow_html=True)
